...
<head>
    ...
    <link rel="icon" type="image/png" sizes="32x32" href="{{asset('assets/front-end/icons/Logo.svg')}}">
    ...
    <link
        ...
        <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&family=Noto+Kufi+Arabic:wght@400;500;600;700&family=Stoke:wght@300;400&display=swap" rel="stylesheet">
    {{-- light box --}}
...
@font-face {
        font-family:"shamelFamilyBold";
        font-weight: 700px;
        src: url("{{asset('assets/front-end/fonts/FFShamelFamily-SansOneBold.ttf')}}") format('truetype');
    }
    @font-face {
        font-family:"shamelFamilyNormal";
        font-weight: 400px;
        src: url("{{asset('assets/front-end/fonts/FFShamelFamily-SansOneBook.ttf')}}") format('truetype'); 
    }
      ...
      * {
      ...
      .shamelFamilyBold {font-family:"shamelFamilyBold";}
    }
  </style>
...
<!-- Page Content-->
<div class="relative overlay-element">
    <!-- Start search input box -->
<!-- This example requires Tailwind CSS v2.0+ -->
<div class="search-box absolute top-0 right-0 bottom-0 left-0 z-10 sm:px-6 lg:px-8" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  <div class="absolute top-0 right-0 left-0 bottom-0 items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0"
  >
    <div
      class="absolute inset-0 top-0 bg-[#201B3DD9] bg-opacity-75 transition-opacity"
      aria-hidden="true"
    ></div>

    <div dir="rtl" style="transform: translateX(-50%)"
      class="
        search-input-wrap           
        absolute
        m-auto
    ...
    >
      <div class="w-full px-20">
          <div  class=" flex flex-row items-center justify-start">
            ...
            >
              <path
                d="M13.5689 27.1266C16.5824 27.1267 19.5096 26.1213 21.887 24.2696L30.8561 33.2386C31.5261 33.8857 32.5937 33.8672 33.2408 33.1971C33.8721 32.5435 33.8721 31.5074 33.2408 30.8539L24.2717 21.8848C28.8666 15.97 27.7966 7.45026 21.8818 2.85541C15.967 -1.73943 7.44733 -0.669453 2.85248 5.24535C-1.74236 11.1602 -0.672383 19.6799 5.24242 24.2747C7.62367 26.1246 10.5535 27.1281 13.5689 27.1266ZM6.36237 6.36011C10.3425 2.37992 16.7955 2.37985 20.7757 6.35997C24.7559 10.3401 24.756 16.7931 20.7759 20.7733C16.7958 24.7535 10.3427 24.7536 6.36252 20.7735C6.36245 20.7734 6.36245 20.7734 6.36237 20.7733C2.38225 16.8222 2.3588 10.3927 6.30992 6.41256C6.32738 6.39503 6.34484 6.37757 6.36237 6.36011Z"
                fill="#201A3C"
              />
            </svg>
            <!-- <input dir="rtl" class="max-w-8 max-h-8 text-[#201A3C] text-[24px] pr-3 font-shamelnormal" />   -->
               
            
            <form action="{{route('products')}}" type="submit" class="search_form relative">
            ...
            <input
                ...
                />
                        ...
                        <button class="input-group-append-overlay hidden search_button" type="submit"
                        ...
                        <div class="card absolute bg-white search-card z-10 h-40 overflow-auto md:w-[350px] w-[270px] top-[100%] right-0 rounded shadow"
                             style="display: none">
                            <div class="card-body search-result-box p-3 text-right"
                                 style="overflow:scroll;overflow-x: hidden"></div>
                        </div>
                    </form>
...
<script src="{{asset('assets/front-end')}}/js/jquery.star-rating.js"></script>
...
{{--<script>
        ...
        $("#close-register-compleate").click(function(){
            $("#popup-modal-compleate-register").hide();
        });
        $("#close-reset-password").click(function(){
            $("#popup-modal-reset-password").hide();
        });

        $("#close-reset-new-password").click(function(){
            $("#popup-modal-reset-new-password").hide();
        });
        ...
        $("#new-register").click(function(){
            $("#new-register").last().addClass("text-white bg-[#CC9933]");
            $("#login").last().addClass("hover:text-[#CC9933]");
            $("#login").removeClass('text-white bg-[#CC9933]');
            $("#new-register").removeClass('hover:text-[#CC9933]');
            $("#login-form").hide();
            $("#register-form").show();
        ...
        $("#login").click(function(){
            $("#login").last().addClass("text-white bg-[#CC9933]");
            $("#new-register").last().addClass("hover:text-[#CC9933]");
            $("#new-register").removeClass('text-white bg-[#CC9933]');
            $("#login").removeClass('hover:text-[#CC9933]');
            $("#login-form").show();
            $("#register-form").hide();
        });
...
<script>
    ...
    function addToCartAndBuyNow(form_id = 'add-to-cart-form') {
        if (checkAddToCartValidity()) {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
    ...
    function removeFromCartNotLoginShopCart(key) {
        $.post('{{ route('cart.remove-without-login') }}', {_token: '{{ csrf_token() }}', key: key}, function (response) {
    ...
    function removeFromCart(key) {
        $.post('{{ route('cart.remove') }}', {_token: '{{ csrf_token() }}', key: key}, function (response) {
            ...
            updateNavCart();
            $('#cart-summary').empty().html(response.data);
            toastr.info('{{\App\CPU\translate('تمت إزالة العنصر من سلة التسوق')}}', {
                CloseButton: true,
                ProgressBar: true
            });
        });
        ...
        $('.input-number').change(function () {
            ...
            if (valueCurrent >= minValue) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Cart',
                    text: '{{\App\CPU\translate('عذرا ، تم الوصول إلى الحد الأدنى للقيمة')}}'
                });
            ...
            if (valueCurrent <= maxValue) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Cart',
                    text: '{{\App\CPU\translate('عذرا ، تجاوزت حدود المخزون')}}.'
                });
                $(this).val($(this).data('oldValue'));
            }
    ...
    function checkOutOfStock(){
        ...
        var color_name = '';
        document.querySelectorAll(".radio-color-circle input[name='color']").forEach(function(color) {
        
                
                if(color.type=="radio" && color.name=="color") {
                
                    if(color.checked){
                        color_name = color.getAttribute("data-name");
                        var selected_color = document.getElementById("selected_color");
                        selected_color.value = color_name;
     ...
     var dataChoiceOptions = choiceOptions.data("choiceoptions");
     var dataVariation = choiceOptions.data("variation");
    
    
    
    for(var i=0;i<dataChoiceOptions.length;i++){

     
    var options = dataChoiceOptions[i].options;
    ...
    var option  = dataChoiceOptions[i].options[j];
   
    str = str.replaceAll(' ', '');  
    
    if (str != null) {
        str += '-' + option.replaceAll(' ', '');
    } else {
        str += option.replaceAll(' ', '');
    }
        ...
        if (str != null) {
            ...
            var countVariation = dataVariation.length;
            
            for (var k = 0; k < countVariation; k++) {
                
        
                if (dataVariation[k].type == str) {
                   
                    if (dataVariation[k].qty < 1) {
        ...
        var liOption =  $("#li-option-"+(option.replaceAll(' ','')))
       
        if(out_of_stock == true){
            
           if(liOption[0] != null) {
              var lable = liOption[0].getElementsByTagName("label");
              if(lable[0] != null){
                  lable[0].classList.add('bg-[#C4C4C4]')
              }
               var outOfStack =  '<div class="out-of-stack absolute  rounded w-[100%]"> <img class= "mx-auto" src="{{asset('assets/front-end/icons/out-of-stock 2.svg')}}"/> </div>';
               liOption.append(outOfStack);
               var inputOption = liOption[0].getElementsByTagName("input")[0];
            ...
            if(liOption[0] != null) {
                var lable = liOption[0].getElementsByTagName("label");
              if(lable[0] != null){
                  lable[0].classList.remove('bg-[#C4C4C4]')
              }
                var outOfStack = liOption[0].getElementsByClassName('out-of-stack');
                if(outOfStack[0] != null){
                 outOfStack[0].remove();
...
<script>
    ...
    jQuery(".search-bar-input").keyup(function () {
        ...
        $(this).siblings('.search-card').css("display", "block");
    ...
    jQuery(".search-bar-input-product").keyup(function () {
        /* $(".search-card").css("display", "block"); */
        $(this).siblings('.search-card').css("display", "block");
        let name = $(".search-bar-input-product").val();
        if (name.length > 0) {
            $.get({
                url: '{{url('/')}}/searched-products',
                dataType: 'json',
    ...
    function route_alert(route, message) {
        Swal.fire({
            title: '{{\App\CPU\translate('هل تريدين الحذف؟')}}',
            text: message,
            type: 'warning',
