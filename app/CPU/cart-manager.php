...
class CartManager
{
    ...
    public static function cart_grand_total($cart_group_id = null)
    {
        ...
        if (!empty($cart)) {
            foreach ($cart as $item) {
                $product_subtotal = (($item['price']+$item['tax'])  * $item['quantity']);
                $total += $product_subtotal;
            }
            $total += $shipping_cost;
        }
    ...
    public static function add_to_cart($request, $from_api = false)
    {
        ...
        if ($user == 'offline') {
            if (session()->has('offline_cart')) {
                ...
                if (isset($check) == false) {
                    ...
                    return [
                        'status' => 0,
                        'message' => translate('تم الاضافة مسبقا!')
                    ];
                }
            } else {
                $cart = collect();
        ...
        if ($product['current_stock'] < $request['quantity']) {
            return [
                'status' => 0,
                'message' => translate('إنتهى من المخزن')
            ];
        }
        ...
        $price_with_tax_without_discount = ($product->unit_price * 1.17);
        // $product_discount = $product->unit_price-(Helpers::get_product_discount($product,$price_with_tax_without_discount));
        $cart['discount'] = Helpers::get_product_discount($product, $price_with_tax_without_discount);
    ...
    public static function update_cart_qty($request)
    {
        ...
        if(auth('customer')->id() != null) {
         ...
         $cart = collect(CartManager::get_cart())->firstWhere('id', $request->key);
        ...
        return [
            'status' => $status,
            'qty' => $qty,
            'message' => $status == 1 ? translate('أضيف بنجاح!') : translate('آسف المخزون محدود')
        ];
    }

    public static function check_item_cart_in_stoke($cart){
        $product = Product::find($cart['product_id']);
        $count = count(json_decode($product->variation));
        $status = 1;
        if ($count) {
        ...
        } else if ($product['current_stock'] < $cart['quantity']) {
            $status = 0;
        }
