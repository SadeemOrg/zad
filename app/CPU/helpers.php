...
//            dd($str);
    ...
    public static function gen_mpdf($view, $file_prefix, $file_postfix)
    {
       ...
       /* $mpdf->AddPage('XL', '', '', '', '', 10, 10, 10, '10', '270', '');*/
        ...
        $stylesheet = file_get_contents(asset('assets/front-end/css/invoice.css'));
        $mpdf_view = $view;
        $mpdf_view = $mpdf_view->render();
        //$mpdf->WriteHTML($stylesheet,\Mpdf\HTMLParserMode::HEADER_CSS);
        $mpdf->WriteHTML($mpdf_view);
        $mpdf->Output($file_prefix . $file_postfix . '.pdf', \Mpdf\Output\Destination::DOWNLOAD);
    }

    public static function gen_mpdf_return($view, $file_prefix, $file_postfix)
    {
        $mpdf = new \Mpdf\Mpdf(['default_font' => 'FreeSerif', 'mode' => 'utf-8', 'format' => [190, 250]]);
       /* $mpdf->AddPage('XL', '', '', '', '', 10, 10, 10, '10', '270', '');*/
        $mpdf->autoScriptToLang = true;
        ...
        //$mpdf->WriteHTML($stylesheet,\Mpdf\HTMLParserMode::HEADER_CSS);
        $mpdf->WriteHTML($mpdf_view);
        $mpdf->Output($file_prefix . $file_postfix . '.pdf', \Mpdf\Output\Destination::INLINE);
    }
}
