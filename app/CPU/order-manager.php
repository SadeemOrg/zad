...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        // $client = new Client();
        // $res = $client->request('GET', 'https://Chita-il.com/RunCom.Server/Request.aspx?APPNAME=run&PRGNAME=ship_create_anonymous&ARGUMENTS=-N27847,-Aמסירה,-N240,-N4,-Aחובה
        // או חובה,-A,-N198 ,-N,-N,-N,-Asuleiman,-A,-Aנצרת,-A,-Aנצרת,-A10,-A,-A,-A,-A0509669191,-A,-A,-A1
        // ,-A,-A,-A,-A,-A,-N,-N,-N,-A,-A,-N,-N,-AXML,-AY,-A,-Nsuliman@gmal.com,-N,-N,-N');
        // $xmlResponce = simplexml_load_string($res->getBody()->getContents());
        // if($res->getStatusCode() == 200){
        // $json = json_encode($xmlResponce);
        // $data = json_decode($json, true);
        // $answer = $data['mydata']['answer'];
        // $ship_create_num = $answer['ship_create_num'];
        // $ship_num_rand = $answer['ship_num_rand'];
        // }
        ...
        $or = [
            ...
            'order_amount' => $order_amount-$shipping_cost > 300 ? $order_amount-$shipping_cost:$order_amount,
            ...
            'shipping_cost' => $order_amount-$shipping_cost > 300 ? 0.0 :$shipping_cost,
        ...
        if ($or['payment_method'] != 'cash_on_delivery') {
            ...
            DB::table('order_transactions')->insert([
                ...
                'seller_is' => $order['seller_is'],
                'order_id' => $order_id,
                'order_amount' => $order_amount-$order['shipping_cost'] > 300 ? $order_amount-$order['shipping_cost']:$order_amount,
                'seller_amount' => $order_amount - $commission,
                'admin_commission' => $commission,
                'received_by' => 'admin',
                'status' => 'hold',
                'delivery_charge' => $order_amount-$order['shipping_cost'] > 300 ? 0.0:$order['shipping_cost'],
                'tax' => $order_summary['total_tax'],
                'delivered_by' => 'admin',
